...
import { generateCitizenId, getIndustrySpecialtyMap } from '../../data/CitizenWorkshopConfig'
...
export class CitizenRoster {
  ...
  addCitizen(name: string): void {
    ...
    const defaultSpecialty = industryMap[defaultIndustry][0]

    const citizen: WorkshopCitizenConfig = {
      id,
      name: name.trim() || (getLocale() === 'vi' ? 'Cư dân mới' : getLocale() === 'en' ? 'New Citizen' : '新居民'),
  ...
  private showItemMenu(btn: HTMLElement, citizenId: string, citizenName: string): void {
    ...
    delItem.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/><path d="M8 6V4a2 2 012-2h4a2 2 012 2v2"/></svg>${getLocale() === 'vi' ? 'Xóa cư dân' : getLocale() === 'en' ? 'Delete' : '删除居民'}`
  ...
  render(): void {
    ...
    if (this.config.citizens.length > 0) {
      ...
      list.appendChild(this.renderGroup(getLocale() === 'vi' ? `Cư dân (${this.config.citizens.length})` : getLocale() === 'en' ? `Citizens (${this.config.citizens.length})` : `居民 (${this.config.citizens.length})`, citizenItems))
    }

...
import { getSlotLabel } from '../../data/CitizenWorkshopConfig'
...
function getSlotLabels(): Record<string, string> {
  return Object.fromEntries(['idle', 'walk', 'typing', 'wave', 'cheer', 'reading', 'frustrated', 'dancing'].map(k => [k, getSlotLabel(k)]))
}
...
export class CharacterModelUpload {
  ...
  private showStep1(): void {
    ...
    panel.innerHTML = `
      <div class="upload-header">
        <span class="upload-title">${l('Thêm nhân vật', 'Add Character', '添加角色模型')}</span>
      ...
      <div class="upload-dropzone" id="char-upload-dropzone">
        ...
        <div class="dropzone-text">${l('Thả file .glb / .gltf vào đây', 'Drop .glb / .gltf file here', '拖拽 .glb / .gltf 文件到此处')}</div>
        <div class="dropzone-hint">${l('hoặc bấm để chọn file (≤ 50MB)', 'or click to browse (≤ 50MB)', '或点击选择文件（≤ 50MB）')}</div>
      ...
      <button class="ai-gen-toggle" id="char-ai-gen-toggle">
        ...
        <span class="ai-gen-label">${l('AI tạo 3D', 'AI Generate 3D', 'AI 生成 3D 角色')}</span>
  ...
  private validateAndProceed(file: File): void {
    ...
    if (!ALLOWED_EXTENSIONS.includes(ext)) {
      errorEl.textContent = l('Chỉ hỗ trợ .glb / .gltf', 'Only .glb / .gltf supported', '仅支持 .glb / .gltf 格式')
      errorEl.classList.add('visible')
      return
    }

    if (file.size > MAX_FILE_SIZE) {
      errorEl.textContent = l('File vượt quá 50MB', 'File exceeds 50MB', '文件超过 50MB 限制')
      errorEl.classList.add('visible')
      return
    }
  ...
  private renderStep2Form(name: string, scale: number): void {
    ...
    panel.innerHTML = `
      <div class="upload-header">
        <span class="upload-title">${isEdit ? l('Sửa nhân vật', 'Edit Character', '编辑角色模型') : l('Thêm nhân vật', 'Add Character', '添加角色模型')}</span>
        <button class="upload-close-btn">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
        </button>
      </div>
      <div class="upload-body">
        <div class="upload-form">
          <label class="upload-label">
            ${l('Tên', 'Name', '名称')}
            <input type="text" class="upload-input" id="char-upload-name" value="${this.escHtml(name)}" maxlength="20" placeholder="${l('Tên nhân vật', 'Character name', '角色名称')}">
          </label>
          ...
          <label class="upload-optimize-label" id="char-optimize-row">
            ...
            <span class="upload-optimize-text">
              <span class="upload-optimize-name">${l('Tối ưu', 'Optimize', '优化模型')}</span>
              <span class="upload-optimize-hint">${l('Gộp trùng và nén nhẹ file', 'Deduplicate & compress', '去重 + 量化压缩，减小文件体积')}</span>
            </span>
          </label>
          <div class="upload-footer">
            <button class="upload-btn upload-btn-cancel" id="char-upload-cancel">${l('Hủy', 'Cancel', '取消')}</button>
            <button class="upload-btn upload-btn-save" id="char-upload-save">${isEdit ? l('Lưu sửa đổi', 'Save', '保存修改') : l('Lưu', 'Save', '保存')}</button>
          </div>
        </div>
  ...
  private async handleSave(): Promise<void> {
    ...
    if (!name) {
      errorEl.textContent = l('Nhập tên nhân vật', 'Enter a name', '请输入角色名称')
      errorEl.classList.add('visible')
      return
    }
    ...
    try {
      ...
      errorEl.textContent = isEdit ? l('Lưu thất bại, thử lại', 'Save failed', '保存失败，请重试') : l('Tải lên thất bại, thử lại', 'Upload failed', '上传失败，请重试')
      errorEl.classList.add('visible')
      saveBtn.disabled = false
      saveBtn.textContent = isEdit ? l('Lưu sửa đổi', 'Save', '保存修改') : l('Lưu', 'Save', '保存')
    }
  }
  ...
  private async maybeOptimize(assetId: string, statusBtn: HTMLButtonElement): Promise<void> {
    ...
    statusBtn.textContent = l('Đang tối ưu...', 'Optimizing...', '优化中...')
    try {
      ...
      const d = await r.json()
      if (d.success) {
        statusBtn.textContent = l(`Đã tối ưu (-${d.ratio}%)`, `Optimized (-${d.ratio}%)`, `已优化 (-${d.ratio}%)`)
      }

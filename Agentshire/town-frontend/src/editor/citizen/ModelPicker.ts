...
export class ModelPicker {
  ...
  private createAddCard(): HTMLElement {
    ...
    card.innerHTML = `
      ...
      <div class="cw-model-card-name">${l('Thêm model', 'Add Model', '添加模型')}</div>
    `
  ...
  private renderEmptyState(): void {
    ...
    empty.innerHTML = `
      ...
      <div class="cw-picker-empty-text">${l('Chưa có model tùy chỉnh', 'No custom models yet', '还没有自定义角色模型')}</div>
      <div class="cw-picker-empty-hint">${l('Tải lên model 3D .glb / .gltf', 'Upload .glb / .gltf 3D models', '上传 .glb / .gltf 格式的 3D 角色模型')}</div>
    `
    const btn = document.createElement('button')
    btn.className = 'cw-picker-empty-btn'
    btn.textContent = l('+ Thêm model', '+ Add Model', '+ 添加角色模型')
  ...
  private showCardPopover(anchor: HTMLElement, group: CharacterGroup): void {
    ...
    pop.innerHTML = `
      <button class="popover-item" data-action="edit">${ModelPicker.SVG_EDIT} ${l('Sửa', 'Edit', '编辑')}</button>
      <button class="popover-item" data-action="anim">${ModelPicker.SVG_ANIM} ${l('Animation', 'Animations', '动画映射')}</button>
      <button class="popover-item popover-danger" data-action="delete">${ModelPicker.SVG_DELETE} ${l('Xóa', 'Delete', '删除')}</button>
    `

...
import {
  ANIM_SLOTS, getSlotLabel, autoMatchAnimSlots,
...
export class AnimMappingDialog {
  ...
  private async beginLoading(input: AnimDialogInput): Promise<void> {
    this.setStatus(l('Đang tải model...', 'Loading model...', '正在加载模型...'))

    const modelClips = await this.loadModel(input.meshUrl)

    if (modelClips.length > 0) {
      this.sources.push({ label: l('Có sẵn trong model', 'Embedded', '模型内嵌'), url: input.meshUrl, clips: modelClips.map(c => c.name), removable: false })
      this.allClips.push(...modelClips)
    }

    if (input.animFileUrls && input.animFileUrls.length > 0) {
      for (const url of input.animFileUrls) {
        this.setStatus(l('Đang tải animation...', 'Loading animations...', '正在加载动画文件...'))
    ...
    this.overlay.innerHTML = `
      <div class="anim-dialog">
        <div class="anim-dialog-header">
          <span class="anim-dialog-title">${l('Cấu hình animation', 'Animation Mapping', '动画映射配置')}</span>
          <button class="anim-dialog-close" aria-label="${l('Đóng', 'Close', '关闭')}">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        </div>
        <div class="anim-dialog-body">
          <div class="anim-dialog-left">
            <div class="amd-section-label">${l('Nguồn animation', 'Sources', '动画源')}</div>
            <div class="amd-sources" id="amd-sources"></div>
            <button class="amd-add-btn" id="amd-add-anim">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              ${l('Thêm animation', 'Add Animation', '追加动画文件')}
            </button>
            <div class="amd-divider"></div>
            <div class="amd-section-label">${l('Ánh xạ slot', 'Slot Mapping', '槽位映射')}</div>
          ...
          <div class="anim-dialog-right">
            <div class="amd-preview-wrap" id="amd-preview">
              ...
              <div class="amd-preview-loading" id="amd-loading">${l('Đang tải...', 'Loading...', '加载中...')}</div>
              <div class="amd-slot-tabs" id="amd-slot-tabs"></div>
            </div>
          </div>
        </div>
        <div class="anim-dialog-footer">
          <button class="amd-btn amd-btn-cancel" id="amd-cancel">${l('Hủy', 'Cancel', '取消')}</button>
          <button class="amd-btn amd-btn-confirm" id="amd-confirm">${l('Lưu', 'Save', '确认保存')}</button>
        </div>
      </div>
    `
  ...
  private renderSources(): void {
    ...
    if (this.sources.length === 0) {
      el.innerHTML = `<div class="amd-source-empty">${l('Chưa có nguồn animation', 'No animation sources', '未检测到动画源')}</div>`
      return
    }
    for (const src of this.sources) {
      ...
      if (src.removable) {
        ...
        replaceBtn.title = l('Thay', 'Replace', '替换')
        ...
        badge.textContent = l('Có sẵn', 'Built-in', '内嵌')
  ...
  private renderMapping(): void {
    ...
    for (const slot of ANIM_SLOTS) {
      const label = getSlotLabel(slot) ?? slot
  ...
  private buildCustomSelect(
    ...
    trigger.innerHTML = `<span class="amd-dd-text${!selected ? ' placeholder' : ''}">${selected || l('-- Chưa ánh xạ --', '-- Unmapped --', '-- 未映射 --')}</span>`
    ...
    const buildMenu = () => {
      ...
      emptyOpt.textContent = l('-- Chưa ánh xạ --', '-- Unmapped --', '-- 未映射 --')
    ...
    const select = (val: string) => {
      selected = val
      trigger.innerHTML = `<span class="amd-dd-text${!val ? ' placeholder' : ''}">${val || l('-- Chưa ánh xạ --', '-- Unmapped --', '-- 未映射 --')}</span>`
      close()
      onChange(val)
    }
  ...
  private renderSlotTabs(): void {
    ...
    for (const slot of ANIM_SLOTS) {
      ...
      btn.textContent = getSlotLabel(slot) ?? slot
  ...
  private async handleAddAnimFile(): Promise<void> {
    ...
    if (addBtn) { addBtn.disabled = true; addBtn.textContent = l('Đang tải lên...', 'Uploading...', '上传中...') }
    ...
    if (addBtn) {
      ...
      addBtn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> ${l('Thêm animation', 'Add Animation', '追加动画文件')}`
    }
  }

...
import {
  ...
  getIndustrySpecialtyMap,
  CHARACTERS1_DEFAULT_ANIM_MAPPING,
  ANIM_SLOTS,
  getSlotLabel,
  translateDefaultWorkshopConfigForLocale,
...
function l(vi: string, en: string, zh: string): string {
  const locale = getLocale()
  return locale === 'vi' ? vi : locale === 'en' ? en : zh
}
...
export class CitizenWorkshop {
  ...
  private async loadRemoteData(): Promise<void> {
    ...
    if (fileConfig && !this.loadDraft()) {
      this.config = translateDefaultWorkshopConfigForLocale(fileConfig)
      this.restoreGroupTransformsFromConfig()
  ...
  private initPicker(): void {
    ...
    this.picker.onDelete(async (group) => {
      const ok = await this.showConfirm(l('Xóa model', 'Delete Model', '删除模型'), l(`Xóa "${group.displayName}"? Không thể hoàn tác.`, `Delete "${group.displayName}"? This cannot be undone.`, `确定要删除「${group.displayName}」吗？此操作不可撤销。`))
      if (!ok) return
  ...
  private updatePickerFooter(candidateName: string | null): void {
    ...
    if (currentEl) currentEl.textContent = `${l('Hiện tại: ', 'Current: ', '已选角色：')}${currentName}`
    ...
    if (candidateEl) {
      ...
      candidateEl.textContent = candidateName ? `${l('Đổi sang: ', 'Change to: ', '更换为：')}${candidateName}` : ''
    }
    ...
    if (confirmBtn) {
      ...
      confirmBtn.textContent = isSame ? l('Đang dùng', 'Already current', '已是当前模型') : l('Xác nhận', 'Confirm', '确认更换')
    }
  }
  ...
  private initRoster(): void {
    ...
    this.roster.onDelete(id => {
      ...
      const name = citizen?.name || l('cư dân này', 'this citizen', '该居民')
      this.showConfirm(l('Xóa cư dân', 'Delete Citizen', '删除居民'), l(`Xóa "${name}"?`, `Delete "${name}"?`, `确定要删除「${name}」吗？`)).then(ok => {
        if (!ok) return
  ...
  private renderInspector(): void {
    ...
    if (!this.selection) {
      content.innerHTML = `<div class="cw-empty-state">${l('Chọn một nhân vật', 'Select a character', '选择一个角色')}</div>`
      return
    }
  ...
  private renderUserInspector(el: HTMLElement): void {
    ...
    el.innerHTML = `
      <div class="cw-identity-row">
        <div class="cw-avatar-clickable" id="cw-user-avatar-btn" title="${l('Đổi avatar', 'Change avatar', '点击更换头像')}">
        ...
        <div class="cw-identity-fields">
          <input class="cw-input cw-input-name" id="cw-user-name" value="${this.esc(u.name)}" placeholder="${l('Tên', 'Name', '角色名')}" />
        </div>
      </div>
  ...
  private renderStewardInspector(el: HTMLElement): void {
    ...
    el.innerHTML = `
      <div class="cw-identity-row">
        <div class="cw-avatar-clickable" id="cw-stew-avatar-btn" title="${l('Đổi avatar', 'Change avatar', '点击更换头像')}">
        ...
        <div class="cw-identity-fields">
          <input class="cw-input cw-input-name" id="cw-stew-name" value="${this.esc(s.name)}" placeholder="${l('Tên quản gia', 'Name', '管家名')}" />
        </div>
      </div>
      <div class="cw-field">
        <div class="cw-field-label">${l('Phong cách nhân vật', 'Persona Style', '人设风格')}</div>
        <textarea class="cw-textarea" id="cw-stew-bio" placeholder="${l('Mô tả tính cách, cách nói chuyện...', 'Describe personality, style...', '描述管家的性格特征、说话风格…')}">${this.esc(s.bio)}</textarea>
      </div>
      ...
      <div class="cw-field">
        <div class="cw-field-label">${l('Liên kết OpenClaw', 'OpenClaw Binding', 'OpenClaw 绑定')}</div>
        <div class="cw-agent-auto-badge">
          ...
          <span>${l('Tự liên kết với Agent chính hiện tại', 'Auto-bound to main Agent', '自动绑定当前 OpenClaw 主 Agent')}</span>
        </div>
      </div>
      <div class="cw-section cw-section-conditional">
        <div class="cw-section-title">${l('Ánh xạ animation', 'Animation Mapping', '动画映射')}</div>
  ...
  private renderCitizenInspector(el: HTMLElement, c: WorkshopCitizenConfig): void {
    ...
    el.innerHTML = `
      <div class="cw-avatar-standalone">
        <div class="cw-avatar-clickable" id="cw-cit-avatar-btn" title="${l('Đổi avatar', 'Change avatar', '点击更换头像')}">
      ...
      <div class="cw-persona-block${isCustom ? ' cw-persona-custom' : ' cw-persona-default'}">
        ...
        <div class="cw-persona-header">
          <span class="cw-persona-header-label">${l('Tùy chỉnh tính cách', 'Custom Persona', '自定义人设')}</span>
          <div class="pi-toggle${isCustom ? ' active' : ''}" id="cw-cit-custom-toggle"></div>
        </div>` : ''}
        <div class="cw-field">
          <div class="cw-field-label">${l('Tên', 'Name', '名字')}</div>
          <input class="cw-input cw-input-name" id="cw-cit-name" value="${this.esc(c.name)}" placeholder="${l('Tên nhân vật', 'Name', '角色名')}" ${hasPresetSoul ? 'readonly' : ''} />
        </div>
        <div class="cw-field">
          <div class="cw-field-label">${l('Chuyên môn', 'Specialty', '专业技能')}</div>
          <input class="cw-input" id="cw-cit-specialty" value="${this.esc(c.specialty)}" placeholder="${l('VD: Frontend, quản lý sản phẩm...', 'e.g. Frontend, PM...', '如：前端开发、产品经理…')}" ${hasPresetSoul ? 'readonly' : ''} />
        </div>
        <div class="cw-field">
          <div class="cw-field-label">${l('Giới thiệu ngắn', 'Bio', '一句话介绍')}</div>
          <input class="cw-input" id="cw-cit-bio" value="${this.esc(c.bio)}" placeholder="${l('Mô tả ngắn về tính cách', 'Brief description', '一句话描述角色特点')}" ${hasPresetSoul ? 'readonly' : ''} />
        </div>
        ${isCustom ? `<div class="cw-field">
          <div class="cw-field-label-row">
            <div class="cw-field-label">${l('Tính cách đầy đủ', 'Full Persona', '完整人设')}</div>
            <button class="cw-ai-gen-btn" id="cw-cit-gen-soul"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l1.09 3.26L16.36 6l-3.27 1.09L12 10.36l-1.09-3.27L7.64 6l3.27-1.09L12 2z"/><path d="M5 15l.55 1.64L7.18 17.2 5.55 17.75 5 19.4l-.55-1.65L2.82 17.2l1.63-.56L5 15z"/><path d="M19 11l.55 1.64 1.63.56-1.63.55L19 15.4l-.55-1.65-1.63-.55 1.63-.56L19 11z"/></svg> ${l('AI tạo', 'AI Generate', 'AI 生成')}</button>
          </div>
          <textarea class="cw-textarea" id="cw-cit-custom-soul" placeholder="${l('Mô tả tính cách, cách nói chuyện, cách làm việc...', 'Describe personality, style, work...', '详细描述角色的性格、说话风格、工作方式…')}">${this.esc(c.customSoul ?? '')}</textarea>
        </div>
        <div class="cw-persona-hint">${l('Khi xuất bản sẽ tạo file Soul mới từ nội dung trên', 'A new Soul file will be generated on publish', '发布时将根据以上内容合成新 Soul 文件')}</div>` : ''}
      </div>
      ...
      <div class="cw-field">
        <div class="cw-field-label">${l('Chế độ Agent', 'Agent Mode', 'Agent 模式')}</div>
        <div class="cw-agent-toggle-row" id="cw-cit-agent-toggle-row">
          ...
          <span class="cw-agent-toggle-label" id="cw-cit-agent-label">${c.agentEnabled ? l('Đã bật · Có thể chat sau khi xuất bản', 'Enabled · Chat after publish', '已开启 · 发布后可对话') : l('Đã tắt', 'Disabled', '未开启')}</span>
        </div>
        <div class="cw-agent-hint">${c.agentEnabled ? l('Sau khi xuất bản sẽ tạo sub-agent để chat trực tiếp', 'A sub-agent will be created for direct chat', '发布后将创建常驻子 Agent，用户可与该居民直接聊天') : l('Bật để nhân vật có tính cách AI độc lập', 'Enable for independent AI personality', '开启后该居民将拥有独立 AI 人格，可与用户对话')}</div>
      </div>
      <div class="cw-field">
        <div class="cw-field-label">${l('Gán nhà', 'Assign Home', '分配场景住宅')}</div>
    ...
    const onAgentToggle = () => {
      ...
      agentLabel.textContent = c.agentEnabled ? l('Đã bật · Có thể chat sau khi xuất bản', 'Enabled · Chat after publish', '已开启 · 发布后可对话') : l('Đã tắt', 'Disabled', '未开启')
      agentHint.textContent = c.agentEnabled
        ? l('Sau khi xuất bản sẽ tạo sub-agent để chat trực tiếp', 'A sub-agent will be created for direct chat', '发布后将创建常驻子 Agent，用户可与该居民直接聊天')
        : l('Bật để nhân vật có tính cách AI độc lập', 'Enable for independent AI personality', '开启后该居民将拥有独立 AI 人格，可与用户对话')
      this.saveDraft()
    }
    ...
    this.createDropdown(
      ...
      c.homeId || '', l('Chưa gán', 'Unassigned', '未分配'),
  ...
  private async generateSoulForCitizen(c: WorkshopCitizenConfig, el: HTMLElement): Promise<void> {
    ...
    btn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="cw-spin"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg> ${l('Đang tạo...', 'Generating...', '生成中...')}`
    ...
    btn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l1.09 3.26L16.36 6l-3.27 1.09L12 10.36l-1.09-3.27L7.64 6l3.27-1.09L12 2z"/><path d="M5 15l.55 1.64L7.18 17.2 5.55 17.75 5 19.4l-.55-1.65L2.82 17.2l1.63-.56L5 15z"/><path d="M19 11l.55 1.64 1.63.56-1.63.55L19 15.4l-.55-1.65-1.63-.55 1.63-.56L19 11z"/></svg> ${l('AI tạo', 'AI Generate', 'AI 生成')}`
  ...
  private createCascadeDropdown(
    ...
    const displayText = c.industry && c.specialty
      ...
      : c.industry || l('Chọn ngành / chuyên môn', 'Select industry', '选择行业 / 专业')

    const trigger = document.createElement('button')
    trigger.type = 'button'
    trigger.className = 'cw-dropdown-trigger'
    const hasValue = !!(c.industry && c.specialty)
    trigger.innerHTML = `<span class="cw-dd-text${!hasValue ? ' cw-dd-placeholder' : ''}">${hasValue ? this.esc(displayText) : l('Chọn ngành / chuyên môn', 'Select industry', '选择行业 / 专业')}</span>`
    wrapper.appendChild(trigger)

    const menu = document.createElement('div')
    menu.className = 'cw-dropdown-menu cw-cascade-menu'

    const industryMap = getIndustrySpecialtyMap()
    for (const industry of Object.keys(industryMap)) {
      const specialties = industryMap[industry] ?? [l('Trợ lý tổng quát', 'General Assistant', '通用助手')]
  ...
  private renderModelSettingsSection(
    ...
    if (variantEl) {
      if (group.variants.length > 1) {
        ...
        variantEl.innerHTML = `<div class="cw-field-label">${l('Biến thể 3D', '3D Variants', '3D模型变体')}</div>`
    ...
    if (colorEl) {
      if (group.colors.length > 1) {
        ...
        colorEl.innerHTML = `<div class="cw-field-label">${l('Màu 3D', '3D Colors', '3D模型配色')}</div>`
    ...
    container.innerHTML = `
      <div class="cw-anim-status ${hasMapped ? 'has-anim' : 'no-anim'}">
        ...
        <div class="cw-anim-status-text">
          <div class="cw-anim-status-title">${hasMapped ? `${l('Đã ánh xạ', 'Mapped', '已映射')} ${mappedCount}/${totalSlots}` : l('Chưa có animation', 'No animations', '未配置动画')}</div>
          <div class="cw-anim-status-detail">${hasMapped ? mappedNames : l('Nhân vật sẽ không có hiệu ứng animation trong thị trấn', 'No animation effects in town', '角色在小镇中将没有动画效果')}</div>
        </div>
      </div>
      <button class="cw-anim-edit-btn" id="cw-anim-edit-btn">${hasMapped ? l('Sửa ánh xạ', 'Edit Mapping', '编辑动画映射') : l('Cấu hình ánh xạ', 'Configure Mapping', '配置动画映射')}</button>
    `
  ...
  private renderTransformSection(el: HTMLElement, containerSelector: string): void {
    ...
    container.innerHTML = `
      <div class="cw-section">
        <div class="cw-section-title">${l('Chỉnh model', 'Transform', '模型调整')}</div>
        <div class="cw-transform-grid">
          <label class="cw-transform-label">${l('Tỉ lệ', 'Scale', '缩放')}</label>
          ...
          <label class="cw-transform-label">${l('Xoay X', 'Rotate X', '旋转X')}</label>
          ...
          <label class="cw-transform-label">${l('Xoay Y', 'Rotate Y', '旋转Y')}</label>
          ...
          <label class="cw-transform-label">${l('Xoay Z', 'Rotate Z', '旋转Z')}</label>
          ...
          <label class="cw-transform-label">${l('Lệch X', 'Offset X', '偏移X')}</label>
          ...
          <label class="cw-transform-label">${l('Lệch Y', 'Offset Y', '偏移Y')}</label>
          ...
          <label class="cw-transform-label">${l('Lệch Z', 'Offset Z', '偏移Z')}</label>
        ...
        <button class="cw-btn cw-btn-secondary cw-transform-reset" id="cw-tf-reset">${l('Đặt lại gợi ý', 'Reset', '重置为推荐值')}</button>
      </div>
    `
  ...
  private static readonly DRAFT_VERSION = 4
  ...
  async publish(): Promise<{ success: boolean; error?: string; changeset?: any }> {
    ...
    if (!saveOk) return { success: false, error: l('Lưu nháp thất bại', 'Draft save failed', '保存草稿失败') }
    ...
    try {
      ...
      return { success: false, error: data.error ?? l('Xuất bản thất bại', 'Publish failed', '发布失败') }
    } catch {
      return { success: false, error: l('Lỗi mạng', 'Network error', '网络请求失败') }
    }
  }

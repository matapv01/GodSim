...
const CATEGORY_LABELS_VI: Record<string, string> = {
  custom: 'Của tôi', kaykit: 'Thị trấn', buildings: 'Tòa nhà', vehicles: 'Xe',
  roads: 'Đường', nature: 'Tự nhiên', streetProps: 'Đường phố', tiles: 'Gạch',
  signs: 'Biển hiệu', factory: 'Nhà máy', foodProps: 'Đồ ăn', roofProps: 'Mái',
  basketball: 'Thể thao', other: 'Khác', construction: 'Công trình',
}
...
function getCategories(): { key: Category; label: string }[] {
  const labels = getLocale() === 'vi' ? CATEGORY_LABELS_VI : getLocale() === 'en' ? CATEGORY_LABELS_EN : CATEGORY_LABELS_ZH
...
export class AssetPalette {
  ...
  private renderList(): void {
    ...
    if (groups.length === 0) {
      this.listEl.innerHTML = '<div class="palette-empty">' + l('Chưa có asset', 'No assets', '暂无资产') + '</div>'
      return
    }
    ...
    for (const group of pageGroups) {
      ...
      card.innerHTML = `
        ...
        <div class="asset-card-info">
          ...
          <div class="asset-meta">${group.cells[0]}×${group.cells[1]}${colorCount > 1 ? ` · ${colorCount} ${l('màu', 'colors', '色')}` : ''}</div>
        </div>
      `
      ...
      if (isSelected && colorCount > 1) {
        ...
        for (const color of group.colors) {
          ...
          swatch.title = l(`Màu ${color}`, `Color ${color}`, `颜色 ${color}`)
  ...
  private renderCustomList(): void {
    ...
    if (assets.length === 0) {
      this.listEl.innerHTML = `
        <div class="palette-empty custom-empty">
          ...
          <div>${l('Thêm asset cảnh đầu tiên', 'Add your first scene asset', '添加你的第一个场景资产')}</div>
          <button class="custom-add-btn-empty">${l('+ Thêm asset', '+ Add Asset', '+ 添加资产')}</button>
        </div>
      `
    ...
    for (const asset of pageAssets) {
      ...
      card.innerHTML = `
        ...
        <button class="custom-more-btn" title="${l('Thêm', 'More', '更多')}">
    ...
    if (!addBtn) {
      ...
      addBtn.innerHTML = l('+ Thêm asset', '+ Add Asset', '+ 添加资产')
  ...
  private async confirmDelete(asset: CustomAsset): Promise<void> {
    ...
    document.getElementById('confirm-title')!.textContent = l('Xóa asset', 'Delete Asset', '删除资产')
    document.getElementById('confirm-message')!.textContent =
      l(`Xóa "${asset.name}"? Vật thể đang dùng asset này sẽ không hiển thị.`, `Delete "${asset.name}"? Items using this asset will not display.`, `确定要删除「${asset.name}」吗？删除后地图中使用该资产的物件将无法显示。`)

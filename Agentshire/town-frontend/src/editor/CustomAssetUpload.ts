...
export class CustomAssetUpload {
  ...
  private showStep1(): void {
    ...
    panel.innerHTML = `
      <div class="upload-header">
        <span class="upload-title">${l('Thêm asset', 'Add Asset', '添加资产')}</span>
      ...
      <div class="upload-dropzone" id="upload-dropzone">
        ...
        <div class="dropzone-text">${l('Thả file .glb vào đây', 'Drop .glb file here', '拖拽 .glb 文件到此处')}</div>
        <div class="dropzone-hint">${l('hoặc bấm để chọn file (≤ 10MB)', 'or click to browse (≤ 10MB)', '或点击选择文件（≤ 10MB）')}</div>
        <input type="file" accept=".glb" hidden id="upload-file-input">
      </div>
      <div class="upload-error" id="upload-error"></div>
      <div class="upload-divider"><span class="upload-divider-line"></span><span class="upload-divider-text">${l('hoặc', 'or', '或')}</span><span class="upload-divider-line"></span></div>
      <button class="ai-gen-toggle" id="ai-gen-toggle">
        ...
        <span class="ai-gen-label">${l('AI tạo 3D', 'AI Generate 3D', 'AI 生成 3D 资产')}</span>
  ...
  private validateAndProceed(file: File): void {
    ...
    if (!ALLOWED_EXTENSIONS.includes(ext)) {
      errorEl.textContent = l('Chỉ hỗ trợ .glb', 'Only .glb supported', '仅支持 .glb 格式')
      errorEl.classList.add('visible')
      return
    }

    if (file.size > MAX_FILE_SIZE) {
      errorEl.textContent = l('File vượt quá 10MB', 'File exceeds 10MB', '文件超过 10MB 限制')
      errorEl.classList.add('visible')
      return
    }
  ...
  private showStep2Edit(asset: CustomAsset): void {
    ...
    this.loader.loadAsync(resolvedUrl)
      ...
      .catch(() => {
        ...
        if (info) info.textContent = l('Tải model thất bại', 'Model load failed', '模型加载失败')
      })
  }
  ...
  private renderStep2Form(
    ...
    panel.innerHTML = `
      <div class="upload-header">
        <span class="upload-title">${isEdit ? l('Sửa asset', 'Edit Asset', '编辑资产') : l('Thêm asset', 'Add Asset', '添加资产')}</span>
        <button class="upload-close-btn">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg>
        </button>
      </div>
      <div class="upload-body">
        <div class="upload-form">
          <label class="upload-label">
            ${l('Tên', 'Name', '名称')}
            <input type="text" class="upload-input" id="upload-name" value="${this.escHtml(name)}" maxlength="20" placeholder="${l('Tên asset', 'Asset name', '资产名称')}">
          </label>
          <label class="upload-label">${l('Loại', 'Type', '放置类型')}</label>
          ...
          <label class="upload-label">${l('Tỉ lệ', 'Scale', '缩放')}</label>
          ...
          <label class="upload-label">${l('Xoay X', 'Rotation X', '旋转 X')}</label>
          <div class="upload-scale-row">
          ...
          <label class="upload-label">${l('Xoay Y', 'Rotation Y', '旋转 Y')}</label>
          <div class="upload-scale-row">
          ...
          <label class="upload-label">${l('Xoay Z', 'Rotation Z', '旋转 Z')}</label>
          <div class="upload-scale-row">
          ...
          <div class="upload-footer">
            <button class="upload-btn upload-btn-cancel" id="upload-cancel">${l('Hủy', 'Cancel', '取消')}</button>
            <button class="upload-btn upload-btn-save" id="upload-save">${isEdit ? l('Lưu sửa đổi', 'Save', '保存修改') : l('Lưu', 'Save', '保存')}</button>
          </div>
        </div>
        <div class="upload-preview-area">
          <canvas class="upload-preview-canvas"></canvas>
          <div class="upload-preview-info">${l('Đang tải...', 'Loading...', '加载中...')}</div>
        </div>
      </div>
    `
  ...
  private async handleSave(): Promise<void> {
    ...
    if (!name) {
      errorEl.textContent = l('Nhập tên asset', 'Enter a name', '请输入资产名称')
      errorEl.classList.add('visible')
      return
    }
    ...
    saveBtn.textContent = l('Đang lưu...', 'Saving...', '保存中...')
    ...
    try {
      if (this.editingAsset) {
        ...
        if ('error' in result) {
          ...
          saveBtn.textContent = l('Lưu sửa đổi', 'Save', '保存修改')
          return
        }
      } else {
        ...
        if ('error' in result) {
          ...
          errorEl.classList.add('visible')
          saveBtn.disabled = false
          saveBtn.textContent = l('Lưu', 'Save', '保存')
          return
        }
      }
      ...
      errorEl.textContent = l('Lưu thất bại, thử lại', 'Save failed', '保存失败，请重试')
      errorEl.classList.add('visible')
      saveBtn.disabled = false
      saveBtn.textContent = this.editingAsset ? l('Lưu sửa đổi', 'Save', '保存修改') : l('Lưu', 'Save', '保存')
    }
  }
  ...
  private loadPreviewFromFile(file: File): void {
    ...
    this.loader.loadAsync(url)
      ...
      .catch(() => {
        URL.revokeObjectURL(url)
        const info = this.overlay.querySelector('.upload-preview-info') as HTMLElement
        if (info) info.textContent = l('Tải model thất bại, hãy kiểm tra file', 'Model load failed', '模型加载失败，请检查文件是否完整')
      })
  }
  ...
  private static getAssetTypeOptions(): { value: string; label: string }[] {
    return [
      { value: 'building', label: l('Tòa nhà', 'Building', '建筑') },
      { value: 'road', label: l('Đường', 'Road', '道路') },
      { value: 'prop', label: l('Đạo cụ', 'Prop', '道具') },
      { value: 'vehicle', label: l('Xe', 'Vehicle', '汽车') },
      { value: 'nature', label: l('Tự nhiên', 'Nature', '自然') },
      { value: 'streetProp', label: l('Đường phố', 'Street', '街道') },
      { value: 'tile', label: l('Gạch', 'Tile', '瓦片') },
      { value: 'sign', label: l('Biển hiệu', 'Sign', '标牌') },
      { value: 'factory', label: l('Nhà máy', 'Factory', '工厂') },
      { value: 'food', label: l('Đồ ăn', 'Food', '餐饮') },
      { value: 'roofProp', label: l('Mái', 'Roof', '屋顶') },
      { value: 'basketball', label: l('Thể thao', 'Sports', '球场') },
      { value: 'construction', label: l('Công trình', 'Build', '工地') },
      { value: 'other', label: l('Khác', 'Other', '其他') },
    ]
  }

  private getAssetTypeLabel(value: string): string {
    return CustomAssetUpload.getAssetTypeOptions().find(o => o.value === value)?.label ?? l('Đạo cụ', 'Prop', '道具')
  }

  private renderAssetTypeOptions(current: string): string {
    return CustomAssetUpload.getAssetTypeOptions().map(o =>

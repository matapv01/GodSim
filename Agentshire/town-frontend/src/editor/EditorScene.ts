...
export class EditorScene {
  ...
  enterSnapMode(sourceItem: PlacedItem, callback: (newX: number, newZ: number, newElev: number) => void): void {
    ...
    if (statusTool) statusTool.textContent = l('Hút dính: bấm vào mục tiêu', 'Snap: click target', '吸附模式：点击目标模型')
  ...
  private exitSnapMode(): void {
    ...
    const statusTool = document.getElementById('status-tool')
    if (statusTool) statusTool.textContent = l('Chọn', 'Select mode', '选择模式')
  }
  ...
  enterJoinMode(sourceItem: PlacedItem, callback: (newX: number, newZ: number) => void): void {
    ...
    const statusTool = document.getElementById('status-tool')
    if (statusTool) statusTool.textContent = l('Ghép cạnh: bấm vào cạnh mục tiêu', 'Join: click target side', '拼接模式：点击目标模型的侧面')
  ...
  enterForwardMarkMode(item: PlacedItem, onMarked: (forwardAngle: number) => void): void {
    ...
    statusEl.textContent = l('Đánh dấu hướng trước · Bấm vào đầu xe · ESC hủy', 'Mark front · Click vehicle front · ESC cancel', '标记前方 · 请点击模型的车头位置 · ESC 取消')
  ...
  enterRouteEditMode(item: PlacedItem, onComplete: (waypoints: Array<{x: number; z: number}>) => void): void {
    ...
    statusEl.textContent = l(`Sửa lộ trình · Bấm để thêm (${waypoints.length}) · Chuột phải/Enter xong · ESC hủy`, `Route edit · Click to add (${waypoints.length}) · Right/Enter done · ESC cancel`, `路线编辑 · 点击地面添加航点 (${waypoints.length} 个) · 右键/Enter 确认 · ESC 取消`)
    this.container.appendChild(statusEl)

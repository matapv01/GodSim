...
const STATE_LABELS_VI: Record<string, string> = {
  idle: 'Đang rảnh', walking: 'Đang đi bộ', running: 'Đang chạy', sitting: 'Đang nghỉ',
  typing: 'Đang làm việc', thinking: 'Đang suy nghĩ', celebrate: 'Đang ăn mừng', frustrated: 'Bực bội',
  sleeping: 'Đang ngủ', wave: 'Đang chào', working: 'Đang làm việc', talking: 'Đang trò chuyện',
  waiting: 'Đang chờ', done: 'Đã xong', error: 'Có lỗi',
}

export function localizeState(state: string): string {
  const labels = getLocale() === 'vi' ? STATE_LABELS_VI : getLocale() === 'en' ? STATE_LABELS_EN : STATE_LABELS_ZH

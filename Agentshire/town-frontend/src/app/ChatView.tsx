  ...
  const handleCommand = useCallback((cmd: ParsedCommand) => {
    ...
    switch (cmd.command) {
      ...
      case 'stop': {
        sendAbort()
        setThinking(false)
        addSystemMessage(routingKey, 'Đã gửi yêu cầu dừng.')
        return
      }
      default: {
        sendCommand(cmd.command, cmd.args)
        addSystemMessage(routingKey, `Đang chạy /${cmd.command}${cmd.args ? ' ' + cmd.args : ''}...`)
        return
      }
    }
      ...
      <div className={cn(
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-xs text-text-quaternary">Đang tải...</div>
          </div>
        ) : (
          <AgentList
            agents={agents}
        ...
        {selectedAgent ? (
          <>
            ...
            <ChatInputBar
              ...
              placeholder={
                !agentOnline
                  ? `${selectedAgent.name} đang offline`
                  : thinking
                    ? `${selectedAgent.name} đang nghĩ...`
                    : `Nhắn gì đó với ${selectedAgent.name}...`
              }
            />
          </>
        ) : (

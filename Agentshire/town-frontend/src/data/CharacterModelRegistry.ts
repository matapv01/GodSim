...
function getBuiltinDisplayName(id: string): string {
  const parts = id.replace('char-', '').split('-')
  if (getLocale() !== 'vi') return parts.join(' ')
  const gender = parts[0] === 'male' ? 'Nam' : parts[0] === 'female' ? 'Nữ' : 'Nhân vật'
  const variant = parts.slice(1).join(' ').toUpperCase()
...
function ensureBuiltinCache(): void {
  ...
  _builtinGroups = _builtinModels.map(m => ({
    id: m.id,
    displayName: getBuiltinDisplayName(m.id),
    source: 'builtin' as const,
...
function getPetGroups(): CharacterGroup[] {
  return PET_NAMES.map(name => ({
    id: `char-pet-${name}`,
    displayName: getLocale() === 'vi' ? `Pet ${name}` : name,
    source: 'builtin' as const,
...
export function getBuiltinGroups(): CharacterGroup[] {
  ensureBuiltinCache()
  return [..._builtinGroups!, ...getPetGroups()]
}
...
export function getAllGroups(customAssets: CustomAsset[] = []): CharacterGroup[] {
  ...
  return [..._builtinGroups!, ...getPetGroups(), ...LIBRARY_GROUPS, ...customs]
}

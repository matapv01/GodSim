...
export class EncounterManager {
  ...
  private getAllNpcs: (() => NPC[]) | null = null
  ...
  setNpcListAccessor(fn: () => NPC[]): void {
    this.getAllNpcs = fn
  }
  ...
  private async runTurn(
    ...
    if (this.dialogueProvider) {
      try {
        const nearbyAudience = this.getNearbyAudience(speaker, listener)
    ...
    if (!text) {
      if (this.dialogueProvider) return true
      text = pick(scene === 'encounter_init' ? getFallbackInit() : getFallbackReply())
    }
  ...
  private getNearbyAudience(speaker: NPC, listener: NPC, radius = 3.2): Array<{ id: string; name: string; distanceToSpeaker: number; distanceToListener: number }> {
    const npcs = this.getAllNpcs?.() ?? []
    return npcs
      .filter(n => n.mesh.visible && n.id !== speaker.id && n.id !== listener.id && n.id !== 'steward' && n.id !== 'user')
      .map(n => {
        const ps = speaker.getPosition()
        const pl = listener.getPosition()
        const pn = n.getPosition()
        const dsx = ps.x - pn.x
        const dsz = ps.z - pn.z
        const dlx = pl.x - pn.x
        const dlz = pl.z - pn.z
        return {
          id: n.id,
          name: n.label ?? n.id,
          distanceToSpeaker: Math.sqrt(dsx * dsx + dsz * dsz),
          distanceToListener: Math.sqrt(dlx * dlx + dlz * dlz),
        }
      })
      .filter(n => Math.min(n.distanceToSpeaker, n.distanceToListener) <= radius)
      .sort((a, b) => Math.min(a.distanceToSpeaker, a.distanceToListener) - Math.min(b.distanceToSpeaker, b.distanceToListener))
      .slice(0, 5)
  }

...
import { getProfessionForSpecialty } from '../data/Professions'
...
export function generateRouteProfile(npcId: string, homeBuilding: string, specialty?: string): NPCRouteProfile {
  ...
  for (const b of BUILDING_REGISTRY) {
    ...
    if (profession.affinity[b.key]) w *= profession.affinity[b.key]
    if (profession.workplaceKeys.includes(b.key)) w *= 1.8
    if (profession.patrolKeys?.includes(b.key)) w *= 1.4
    if (profession.workplaceKeys.length > 0 && b.tag === 'home' && b.key !== homeBuilding) w *= 0.05
    affinities[b.key] = w
    total += w
  }
...
export class DailyBehavior {
  ...
  goToPlaceNow(buildingKey: string, detail = 'Đi tới điểm hẹn'): void {
    const allowedKey = this.resolveAllowedBuildingKey(buildingKey)
    if (!allowedKey) {
      this.goHomeNow('Không vào nhà người khác khi không có lời mời rõ ràng')
      return
    }
    this._inDialogue = false
    this._dialoguePausedState = null
    ...
    this.pendingHome = false
    this.npc.setVisible(true)
    const building = BUILDING_REGISTRY.find(b => b.key === allowedKey)
    this.journal?.record({
      location: allowedKey,
      locationName: building ? building.name : allowedKey,
      action: 'walking',
      detail,
    })
    this.transitionTo('roaming')
    this.walkToBuilding(allowedKey)
  }

  walkAwayFrom(origin: { x: number; z: number }, detail = 'Tạm biệt rồi đi tiếp'): void {
    this._inDialogue = false
    this._dialoguePausedState = null
    this.leaveCurrentBuilding()
    this.setDestination(null)
    ...
    dz /= len
    const target = {
      x: Math.max(4, Math.min(52, pos.x + dx * 4 + (Math.random() - 0.5) * 1.2)),
      z: Math.max(4, Math.min(31, pos.z + dz * 4 + (Math.random() - 0.5) * 1.2)),
    }
    this.journal?.record({
      location: 'town',
    ...
    this.transitionTo('roaming')
    this.doWalk('roaming', target, this.profile.walkSpeed ?? 2.5, () => {
      this.selectNextDestination()
    })
  }
  ...
  interrupt(reason: 'summoned'): void {
    ...
    this.journal?.record({
      ...
      locationName: this.currentBuilding ? this.buildingName(this.currentBuilding) : 'Quảng trường',
      action: 'summoned',
      detail: 'Được gọi tới buổi bàn việc',
    })
  ...
  update(deltaTime: number, _allNpcs: NPC[]): void {
      ...
      if (this.wakeTimer <= 0) {
        ...
        if (this.state === 'sleeping') {
          ...
          this.journal?.record({
            ...
            locationName: this.buildingName(this.profile.homeBuilding),
            action: 'woke_up',
            detail: 'Thức dậy và chuẩn bị ra khỏi nhà',
          })
          this.transitionTo('leaving_home')
          this.startWalkFromHome()
        }
      ...
      if (this.homeTimer <= 0) {
        ...
        if (this.isLifeState() && this.state !== 'walking_home' && this.state !== 'sleeping') {
          ...
          this.journal?.record({
            ...
            locationName: this.buildingName(this.profile.homeBuilding),
            action: 'walking',
            detail: 'Trời tối dần, quay về nhà',
          })
          this.transitionTo('walking_home')
          this.startWalkHome()
        }
    ...
    if (this.state === 'at_building') {
      ...
      if (this.stateTimer >= this.stateDuration) {
        ...
        const slot = template ? getScheduleSlotForPeriod(template, this.gameClock.getPeriod()) : null
        if (this.currentBuilding && slot && Math.random() < slot.stayChance * 0.35) {
          this.stateTimer = 0
          this.stateDuration = randRange(8_000, 18_000)
          this.recordStaying()
          return
        }
        this.leaveCurrentBuilding()
  ...
  private pickBuilding(): BuildingDef | null {
    const candidates = BUILDING_REGISTRY.filter(b => {
      ...
      if (b.tag === 'home' && b.key !== this.profile.homeBuilding) return false
  ...
  private enterBuilding(building: BuildingDef): void {
    ...
    this.journal?.record({
      ...
      action: 'arrived',
      detail: this.arrivalDetail(building),
    })
  ...
  private walkToBuilding(buildingKey: string): void {
    ...
    const building = BUILDING_REGISTRY.find(b => b.key === allowedKey)
    const wp = WAYPOINTS[allowedKey]
    if (!building || !wp) {
      this.wanderPark()
      return
    }
    this.setDestination(allowedKey)
    const target = { x: wp.x + (Math.random() - 0.5) * 1.5, z: wp.z + (Math.random() - 0.5) * 1.5 }
    const allocated = this.spotAllocator
      ? this.spotAllocator.allocate(target, this.npc.id, 1.2)
      : target
  ...
  private leaveCurrentBuilding(): void {
    if (this.currentBuilding) {
      ...
      this.journal?.record({
        ...
        action: 'departed',
        detail: `Rời ${this.buildingName(this.currentBuilding)}`,
      })
  ...
  private getInitialAnimForBuilding(b: BuildingDef): string {
    if (b.tag === 'cafe' || b.tag === 'restaurant') return 'sitting'
    if (b.tag === 'office') return Math.random() < 0.55 ? 'typing' : 'thinking'
    if (b.tag === 'clinic') return 'thinking'
    if (b.tag === 'home') return Math.random() < 0.5 ? 'sitting' : 'idle'
  ...
  private resolveAllowedBuildingKey(buildingKey: string): string | null {
    const building = BUILDING_REGISTRY.find(b => b.key === buildingKey)
    if (!building) return null
    if (building.tag === 'home' && building.key !== this.profile.homeBuilding) return null
    return building.key
  }

  private placeInsideLocationZone(buildingKey: string): void {
    const zone = LOCATION_ZONES[buildingKey]
    if (!zone) return
    const pos = this.randomPointInZone(buildingKey, 0.72)
    this.npc.mesh.position.x = pos.x
    this.npc.mesh.position.z = pos.z
  }

  private randomPointInZone(buildingKey: string, padding = 0.45): { x: number; z: number } {
    const zone = LOCATION_ZONES[buildingKey]
    if (!zone) {
      const wp = WAYPOINTS[buildingKey] ?? WAYPOINTS.plaza_center
      return { x: wp.x, z: wp.z }
    }
    const w = Math.max(0.2, zone.w - padding)
    const d = Math.max(0.2, zone.d - padding)
    return {
      x: zone.x + (Math.random() - 0.5) * w,
      z: zone.z + (Math.random() - 0.5) * d,
    }
  }

  private arrivalDetail(building: BuildingDef): string {
    switch (building.tag) {
      case 'office': return `Đến ${building.name} để xử lý công việc`
      case 'restaurant': return `Đến ${building.name} để ăn uống và gặp người quen`
      case 'cafe': return `Ghé ${building.name} nghỉ một chút`
      case 'market': return `Ra ${building.name} mua đồ và nghe tin trong thị trấn`
      case 'clinic': return `Ghé ${building.name} kiểm tra sức khỏe`
      case 'park': return `Ra ${building.name} đi dạo và hóng gió`
      case 'home': return `Về ${building.name} lo chuyện riêng`
      case 'museum': return `Đến ${building.name} xem triển lãm và suy nghĩ`
      default: return `Đến ${building.name}`
    }
  }

  private recordStaying(): void {
    if (!this.currentBuilding) return
    const building = BUILDING_REGISTRY.find(b => b.key === this.currentBuilding)
    if (!building) return

    let detail = `Đang ở ${building.name}`
    switch (building.tag) {
      case 'office': detail = `Đang tập trung làm việc ở ${building.name}`; break
      case 'restaurant': detail = `Đang ăn uống và trò chuyện ở ${building.name}`; break
      case 'cafe': detail = `Đang ngồi nghỉ ở ${building.name}`; break
      case 'market': detail = `Đang mua đồ ở ${building.name}`; break
      case 'clinic': detail = `Đang nghỉ và kiểm tra sức khỏe ở ${building.name}`; break
      case 'park': detail = `Đang đi dạo ở ${building.name}`; break
      case 'home': detail = `Đang lo việc riêng tại ${building.name}`; break
    }

...
describe('SceneBootstrap.loadFinalConfig', () => {
  ...
  it('falls back to localStorage when fetch returns null config', async () => {
    ...
    const localConfig = { townName: '旧镇', steward: { name: 'Old', persona: '', avatarId: 'char-female-b' }, user: { name: '旧镇长', avatarId: 'char-male-c' }, citizens: [], createdAt: 1, version: 5 }
    const deps = createMockDeps()
  ...
  it('falls back when published characters is empty array', async () => {
    ...
    const localConfig = { townName: '本地', steward: { name: 'Local', persona: '', avatarId: 'a' }, user: { name: 'U', avatarId: 'b' }, citizens: [], createdAt: 1, version: 5 }
    const deps = createMockDeps()
    deps.configStore.load.mockReturnValue(localConfig)

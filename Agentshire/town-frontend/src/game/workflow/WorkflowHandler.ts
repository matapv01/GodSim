...
export class WorkflowHandler {
  ...
  async playSummonSequence(): Promise<void> {
    ...
    cameraCtrl.moveTo({ x: WAYPOINTS.plaza_center.x, z: WAYPOINTS.plaza_center.z })
    ui.setProgress(0, citizenIds.length, getLocale() === 'en' ? 'Rallying...' : '团队集结中...')
    await this.delay(500)

    const rallyPositions = [
      { x: 21.6, z: 21.8 }, { x: 24.0, z: 21.8 }, { x: 27.2, z: 21.8 }, { x: 30.0, z: 21.8 },
    ]
    ...
    if (userNpc) userNpc.mesh.position.set(WAYPOINTS.plaza_center.x, 0, WAYPOINTS.plaza_center.z)
    if (stewardNpc) stewardNpc.mesh.position.set(WAYPOINTS.plaza_center.x - 3, 0, WAYPOINTS.plaza_center.z)
  }

      ...
      try {
        ...
        if (msg.type === "town_session_init" && typeof msg.townSessionId === "string") {
          ...
          opts.onImplicitChat({
            ...
            if (result.error) {
              console.warn(`${sessionLogPrefix(townSessionId)} implicit_chat failed: ${result.error}`);
            }
            if (ws.readyState === WebSocket.OPEN) {
              ws.send(JSON.stringify({ type: "implicit_chat_response", id: msg.id, text: result.text, usage: result.usage, error: result.error }));
            }
          }).catch((err) => {
...
export function pushCitizenChatDelta(params: {
  townSessionId: string;
  agentId: string;
  npcId?: string;
  items: Array<{
  ...
  for (const ws of clients) {
    if (ws.readyState === WebSocket.OPEN && getClientSessionId(ws) === params.townSessionId) {
      ws.send(payload);
    }
  }
  const assistantMessages = params.items
  ...
  if (params.npcId && assistantMessages.length > 0) {

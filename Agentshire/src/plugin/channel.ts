...
async function dispatchStewardLlmMessage(params: {
  llmChat: (req: {
    system: string;
    user: string;
    maxTokens: number;
  ...
  const { llmChat, townSessionId, body } = params;
  const userTs = Date.now();
  appendChatItems({
    townSessionId,
    agentId: "steward",
    items: [{
      id: `steward-user:${townSessionId}:${userTs}`,
      agentId: "steward",
      timestamp: userTs,
      kind: "text",
      role: "user",
  ...
  broadcastAgentEvent({ type: "system", subtype: "init", message: "Steward is thinking." }, townSessionId);
  broadcastAgentEvent({ type: "thinking_delta", content: "Đang suy nghĩ..." }, townSessionId);
  ...
  appendChatItems({
    ...
    agentId: "steward",
    items: [{
      id: `steward-assistant:${townSessionId}:${Date.now()}`,
      agentId: "steward",
      timestamp: Date.now(),
      kind: "text",
      role: "assistant",
  ...
  broadcastAgentEvent({ type: "text", content: reply }, townSessionId);
  broadcastAgentEvent({ type: "system", subtype: "done", message: "Steward replied." }, townSessionId);
}
    ...
    startAccount: async (ctx: any) => {
      ...
      startTownWsServer({
        ...
        onChat: async ({ message, townSessionId }) => {
          ...
          try {
            await dispatchStewardLlmMessage({
              llmChat,

...
import { loadLongTermChatItems } from "./longterm-log.js";
...
export function loadChatItemHistory(
  ...
  allItems.push(...loadLongTermChatItems("steward", limit));
...
export function loadCitizenItemHistory(
  ...
  if (!existsSync(sessDir)) {
    const longTermItems = loadLongTermChatItems(agentId, limit);
    return { items: longTermItems, hasMore: false, cursor: "" };
  }

  try {
    const allItems: ChatItem[] = [];
    allItems.push(...loadLongTermChatItems(agentId, limit));
    const indexedIds = new Set<string>();
    const prefix = `agent:${agentId}:`;
